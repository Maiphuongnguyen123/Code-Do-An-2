...
//     // const [searchQuery, setSearchQuery] = useState('');
    ...
    const AgentsGird = (props) => {
        // State quản lý dữ liệu và giá trị tìm kiếm
        const [tableData, setTableData] = useState(mockRentaler);
        const [searchQuery, setSearchQuery] = useState("");
        const [locationQuery, setLocationQuery] = useState("");
        const [roomSlotQuery, setRoomSlotQuery] = useState("");
    
        // Lọc dữ liệu khi giá trị tìm kiếm thay đổi
        useEffect(() => {
            const filteredData = mockRentaler.filter((rentaler) => {
                const matchesName = rentaler.name.toLowerCase().includes(searchQuery.toLowerCase());
                const matchesLocation = locationQuery
                    ? rentaler.area.toLowerCase().includes(locationQuery.toLowerCase())
                    : true;
                const matchesRoomSlot =
                    roomSlotQuery === "3+"
                        ? parseInt(rentaler.roomslot) >= 3
                        : roomSlotQuery
                        ? rentaler.roomslot === roomSlotQuery
                        : true;
                return matchesName && matchesLocation && matchesRoomSlot;
            });
            setTableData(filteredData);
        }, [searchQuery, locationQuery, roomSlotQuery]);
        return (
                            ...
                            <div className="row">
                                <div className="col-md-12 col-lg-8">
                                    <div className="title-single-box">
                                        ...
                                        <span className="color-text-a">Tìm thông tin của người cho thuê tại đây</span>
                                    </div>
                                </div>
                        ...
                        <div className="row mb-4">
                            ...
                            <div className="col-md-4">
                                <select
                                    ...
                                    value={roomSlotQuery}
                                    onChange={(e) => setRoomSlotQuery(e.target.value)}
                                >
                                    <option value="">Số phòng trống...</option>
                                    <option value="0">Không còn phòng</option>
